...
def branch_dag():
    ...
    #Creating the Decider Node to decide which path to take based on the weekend_flag value
    @task.branch
    def decider_task(**kwargs):
        ...
        if weekend_flag == "true":
            return "no_load_task" # should match(task id) the value that is passed in the load_task's bash command
        else:
            return "load_task"  # should match(task id) the value that is passed in the load_task's bash command
    ...
    extract_task() >> [transform_task_api(), transform_task_db(), transform_task_file()] >> decider_task() >> [no_load_task(), load_task()]
